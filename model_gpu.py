...
class CausalSelfAttention:
    ...
    def forward(self, x: cp.ndarray) -> cp.ndarray:
        ...
        self.q_t, self.k_t, self.v_t = cp.split(self.c_attn.forward(x), 3, axis=2)

        self.k_t = cp.transpose(self.k_t.reshape(self.B, self.T, self.n_head, self.C // self.n_head), (0, 2, 1, 3)) # (B, nh, T, hs)
        self.q_t = cp.transpose(self.q_t.reshape(self.B, self.T, self.n_head, self.C // self.n_head), (0, 2, 1, 3)) # (B, nh, T, hs)
        self.v_t = cp.transpose(self.v_t.reshape(self.B, self.T, self.n_head, self.C // self.n_head), (0, 2, 1, 3)) # (B, nh, T, hs)
        # causal self-attention; Self-attend: (B, nh, T, hs) x (B, nh, hs, T) -> (B, nh, T, T)

        # manual implementation of attention
        self.att = cp.matmul(self.q_t,  cp.transpose(self.k_t, (0, 1, 3, 2))) * (1.0 / math.sqrt(self.k_t.shape[-1]))
        # causal mask to ensure that attention is only applied to the left in the input sequence
        self.att = cp.where(cp.tril(self.att, self.T - self.att.shape[-1]) != 0, self.att, float('-inf'))
        self.att = self.softmax.forward(self.att)
        self.att = self.attn_dropout.forward(self.att)
    ...
    def backward(self, grad: cp.ndarray) -> cp.ndarray:
        ...
        y_g = grad.reshape(self.B, self.T, self.n_head, self.C // self.n_head)
        y_g = cp.transpose(y_g, (0, 2, 1, 3))

        v_g = cp.matmul(cp.transpose(self.att, (0, 1, 3, 2)), y_g) # QK^T @ y_g -> v_g: (B, nh, T, T) x (B, nh, T, hs) -> (B, nh, T, hs)
        grad = cp.matmul(y_g, cp.transpose(self.v_t, (0, 1, 3, 2))) # y_g @ v -> v_g: (B, nh, T, T) x (B, nh, T, hs) -> (B, nh, T, hs)

        grad = self.attn_dropout.backward(grad)
        grad = self.softmax.backward(grad)
        grad = cp.where(cp.tril(grad, self.T - grad.shape[-1]) != 0, grad, 0)

        q_t_g = (1.0 / math.sqrt(self.k_t.shape[-1])) * (cp.matmul(grad, self.k_t))
        k_t_g = (1.0 / math.sqrt(self.k_t.shape[-1])) * (cp.matmul(cp.transpose(grad, (0, 1, 3, 2)), self.q_t)) # (Q^T @ att)^T = att^T @ Q
...
class GPT:
    ...
    def generate(self, idx: np.ndarray, max_new_tokens, temperature=1.0, top_k=None)-> np.ndarray:
        ...
        for _ in tqdm(range(max_new_tokens)):
            ...
            logits, _, _ = self.forward(cp.asarray(idx_cond), calculate_norms=False)
